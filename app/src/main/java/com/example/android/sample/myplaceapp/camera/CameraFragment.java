...
/**
 * API Level21以降用のカメラ機能を実装するFragment。
 */
...
public class CameraFragment extends Fragment implements View.OnClickListener {
    ...
    private void openCamera(int width, int height) {
        ...
        mImageReader = ImageReader.newInstance(mCameraInfo.getPictureSize().getWidth(),
                ...
                ImageFormat.JPEG,
                2);
        // 画像が得られるたびに呼ばれるリスナとそのリスナが動作するスレッドを設定する
        ...
        try {
            manager.openCamera(mCameraInfo.getCameraId(),
                    mStateCallback,
                    mBackgroundHandler);
    ...
    /**
     * TextureViewのサイズに合わせてプレビューに補正をかけ、画面の向きに合わせてプレビューを回転させる。
     *
     * @param viewWidth
     * @param viewHeight
     */
    private void transformTexture(int viewWidth,int viewHeight){
        ...
        Size viewSizew = new Size(viewWidth,viewHeight);

        Matrix matrix = new Matrix();

        // TextureViewのサイズを示す矩形
        RectF viewRect = new RectF(0,0, viewSizew.getWidth(),viewSizew.getHeight());
        ...
        if(rotation == Surface.ROTATION_90 || rotation == Surface.ROTATION_270){
            ...
            // 拡大・縮小率を決定する
            float scale = Math.max(viewSizew.getHeight() / mCameraInfo.getPictureSize().getHeight(),
                    viewSizew.getWidth() / mCameraInfo.getPictureSize().getWidth());
