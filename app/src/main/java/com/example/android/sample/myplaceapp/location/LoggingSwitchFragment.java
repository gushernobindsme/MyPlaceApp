 ...
 * 位置情報を記録するFragment。
 */
public class LoggingSwitchFragment extends Fragment implements CompoundButton.OnCheckedChangeListener{
    ...
    /**
     * ユーザに問題を解決してもらえる場合のリクエスト。
     */
    private static final int REQUEST_RESOLBE_PROBLEMS = 2;
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (isChecked){
            // スイッチをONにした場合、位置情報取得のシーケンスを開始
            checkServiceAvailability();
        } else if(mGoogleApiClient != null && mGoogleApiClient.isConnected()){
            // すでにGoogleApiClientに接続済みで、スイッチをOFFにした場合、位置情報のリクエストをキャンセル
            LocationServices.FusedLocationApi.removeLocationUpdates(mGoogleApiClient,getLocationPendingIntent(PendingIntent.FLAG_UPDATE_CURRENT));
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(requestCode == REQUEST_INSTALL_OR_UPDATE || requestCode == REQUEST_RESOLBE_PROBLEMS){
            if(requestCode == Activity.RESULT_OK){
                // インストールやアップデートによって、Google Play開発者サービスが利用可能になった場合
                onGooglePlayServicesAvailable();
            }else{
                // 利用可能にならなかったら、終了
                handleError();
    ...
    private void onGooglePlayServicesAvailable(){
        mGoogleApiClient = new GoogleApiClient.Builder(getActivity())
                .addApi(LocationServices.API) // 位置情報のAPI
                .addConnectionCallbacks(mGoogleApiClientCallback) // 接続・切断のイベントを受け取るコールバックを指定
                .addOnConnectionFailedListener(mConnectionFailedListener) // 接続失敗イベントを受け取るコールバックを指定
                .build();
     ...
     */
    private GoogleApiClient.ConnectionCallbacks mGoogleApiClientCallback = new GoogleApiClient.ConnectionCallbacks() {
    ...
    private GoogleApiClient.OnConnectionFailedListener mConnectionFailedListener = new GoogleApiClient.OnConnectionFailedListener() {
        ...
        public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
            if (connectionResult.hasResolution()){
                ...
                try {
                    // 解決するためのアクティビティを起動する
                    connectionResult.startResolutionForResult(getActivity(),REQUEST_RESOLBE_PROBLEMS);
    ...
    private void checkUserLocationAvailability(){
        ...
        mLocationRequest = new LocationRequest()
                ...
                .setPriority(LocationRequest.PRIORITY_BALANCED_POWER_ACCURACY); // 省電力性と制度のバランス

        // この位置情報のリクエストは可能かを確認するためのオブジェクトを生成する
        LocationSettingsRequest checkRequest = new LocationSettingsRequest.Builder()
                .addLocationRequest(mLocationRequest) // 確認対象のリクエスト
                .build();

        // ユーザの設定をチェックする
        PendingResult<LocationSettingsResult> result = LocationServices.SettingsApi.checkLocationSettings(mGoogleApiClient,checkRequest);
    ...
    private void onUserLocationAvailable(){
        ...
        if(mGoogleApiClient.isConnected()){
            // パーミッションをチェックする
            if(ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED // 高精度位置情報パーミッション
                    && ContextCompat.checkSelfPermission(getActivity(),Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED){ // 低精度位置情報パーミッション
